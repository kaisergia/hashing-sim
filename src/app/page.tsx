...
export default function Home() {
  ...
  // ...existing code...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-center">
      <div className="bg-gray-800/90 shadow-2xl rounded-3xl p-8 w-full max-w-2xl flex flex-col items-center border border-gray-700">
        <h1 className="text-4xl font-extrabold mb-8 text-blue-300 tracking-tight text-center">
          Hash Table Visualizer
        </h1>

        {/* Insert Section */}
        <div className="mb-8 w-full flex gap-3 items-center">
          <input
            type="number"
            ...
            disabled={searching}
            className="border-2 border-blue-700 focus:border-blue-400 bg-gray-900 text-blue-100 placeholder-blue-400 rounded-xl p-3 w-44 transition"
          />
          <button
            onClick={handleInsert}
            disabled={searching}
            className="bg-blue-700 hover:bg-blue-600 transition text-white px-6 py-2 rounded-xl font-semibold shadow disabled:opacity-50"
          >
            Insert
          </button>
          <button
            ...
            disabled={searching}
            className="bg-indigo-700 hover:bg-indigo-600 transition text-white px-6 py-2 rounded-xl font-semibold shadow disabled:opacity-50"
          >
            Random Fill
          </button>
        </div>
        ...
        <div className="mb-8 w-full flex gap-3 items-center">
          <input
            ...
            disabled={searching}
            className="border-2 border-green-700 focus:border-green-400 bg-gray-900 text-green-100 placeholder-green-400 rounded-xl p-3 w-44 transition"
          />
          <select
            value={searchMode}
            onChange={(e) => setSearchMode(e.target.value as SearchMode)}
            disabled={searching}
            className="border-2 border-gray-700 rounded-xl p-3 bg-gray-900 text-gray-100 transition"
          >
          ...
          <button
            ...
            disabled={searching}
            className="bg-green-700 hover:bg-green-600 transition text-white px-6 py-2 rounded-xl font-semibold shadow disabled:opacity-50"
          >
            Search
          </button>
        </div>
        ...
        <div className="grid grid-cols-5 sm:grid-cols-10 gap-3 mb-6 w-full">
          {table.map((val, i) => (
            <motion.div
              key={i}
              className={`h-16 flex items-center justify-center border-2 rounded-2xl font-bold text-xl shadow-sm transition
        ...
        {searchMode === 'binary' && (
          <div className="mb-4 w-full">
            <h2 className="font-semibold mb-2 text-gray-300 text-center">Sorted array for binary search:</h2>
        ...
        {/* Result message */}
        {searchResult && (
          <div className="text-lg font-semibold mt-6 text-center text-purple-300 bg-purple-900/70 rounded-xl px-4 py-2 shadow">
            {searchResult}
          </div>
        )}
      </div>
      <footer className="mt-8 text-gray-500 text-sm">Modern UI Hash Table Visualizer &copy; 2025</footer>
    </main>
  );
  // ...existing code...
  // ...existing code...
